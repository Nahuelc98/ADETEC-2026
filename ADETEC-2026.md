# ADETEC - 2026
## Curso de capacitación tecnológica

---

**En este curso trabajamos con herramientas como **Python**, **Git**, **GitHub** y **Visual Studio Code (VS Code)**, desde donde realizamos los *commits* y la gestión del control de versiones.**

**Proyecto:** "Mundial 2026"

Este proyecto tiene como objetivo aplicar programación orientada a objetos (clases e instancias) en Python para:
* Crear y gestionar selecciones de fútbol.
* Administrar planteles (agregar o quitar jugadores).
* Simular partidos entre los equipos.
